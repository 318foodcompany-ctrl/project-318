(function(){
  ...
  function renderQueue(){
    const approval=state.tasks.filter(t=>t.status==="ready_for_approval");
    $("queueCount").textContent=approval.length;
    $("queueList").innerHTML=approval.length?approval.map(task=>{const c=state.content.get(task.ai_content_id)||{},o=c.structured_output||{};return `<article class="task" data-task="${task.id}"><header><div><strong>${esc(c.title||pretty(task.content_type))}</strong><div class="status">${esc(pretty(task.content_type))} · ready for approval</div></div><span class="pill">${new Date(task.created_at).toLocaleString()}</span></header><pre contenteditable="true" data-output="${task.id}" aria-label="Editable AI draft">${esc(JSON.stringify(o,null,2))}</pre><div class="actions"><button class="approve" data-action="approve" data-task-id="${task.id}">Approve</button><button class="secondary" data-action="edit" data-task-id="${task.id}">Save edits</button><button class="secondary" data-action="regenerate" data-task-id="${task.id}">Regenerate</button><button class="reject" data-action="reject" data-task-id="${task.id}">Reject</button></div></article>`;}).join(""):'<div class="empty">Nothing is waiting for approval.</div>';
  }
  function renderHistory(){
    $("historyList").innerHTML=state.audit.length?state.audit.map(a=>`<article class="task"><header><strong>${esc(pretty(a.action))}</strong><span class="pill">${new Date(a.created_at).toLocaleString()}</span></header><div class="muted">Task ${esc(a.task_id||"—")}</div></article>`).join(""):'<div class="empty">No AI audit history yet.</div>';
  }
  ...
  async function load(){
    ...
    try{
      ...
      const ids=[...new Set(tasks.map(t=>t.ai_content_id).filter(Boolean))];
      state.content.clear();
  ...
  async function saveSetting(type){
    ...
    value.items_per_run=Math.max(1,Math.min(20,Number(value.items_per_run||1)));value.preferred_hour=Math.max(0,Math.min(23,Number(value.preferred_hour||8)));value.interval_minutes=value.cadence==="custom_interval"?Math.max(60,Number(value.interval_minutes||60)):null;value.next_run_at=existing?.next_run_at||nextDefault(type);
  ...
  async function action(taskId,action){
    try{
      const session=(await client().auth.getSession()).data.session;if(!session)throw new Error("Session expired.");
      const body={task_id:taskId,action};if(action==="edit")body.structured_output=parsed(taskId);if(action==="reject")body.reason="Rejected by administrator";
      const r=await fetch("/api/admin-marketing-autopilot-action",{method:"POST",headers:{Authorization:`Bearer ${session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify(body)}),payload=await r.json();if(!r.ok)throw new Error(payload.error||"Action failed.");
      msg(action==="approve"?"Approved. Nothing was published or sent.":`${pretty(action)} completed.`);await load();
    }catch(error){msg(`AI task action failed: ${error.message}`,true);}
  }
  async function saveBrain(){
    try{
      const value={mission:$("mission").value.trim(),business_facts:JSON.parse($("businessFacts").value||"{}"),voice_preferences:JSON.parse($("voicePreferences").value||"{}"),seasonal_rules:JSON.parse($("seasonalRules").value||"{}"),growth_priorities:JSON.parse($("growthPriorities").value||"[]"),prohibited_claims:JSON.parse($("prohibitedClaims").value||"[]")};
      await unwrap(client().from("marketing_ai_brand_brain").update(value).eq("id",1).select("*"));msg("Business Brain saved.");await load();
    }catch(error){msg(`Business Brain was not saved: ${error.message}`,true);}
  }
  ...
  async function protect(){const r=await client().auth.getSession(),session=r.data?.session;if(!session){location.replace("login.html");return false;}const a=await client().rpc("crm_is_admin");if(a.error||a.data!==true){location.replace("login.html?error=unauthorized");return false;}$("signedIn").textContent=session.user.email||"Administrator";return true;}
  document.addEventListener("click",e=>{const s=e.target.closest("[data-save-setting]");if(s)saveSetting(s.dataset.saveSetting);const a=e.target.closest("[data-action]");if(a)action(a.dataset.taskId,a.dataset.action);});
