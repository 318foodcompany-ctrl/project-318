(function () {
  ...
  function pipeline() {
    workspace.innerHTML=`<div class="sales-kanban">${utils.stages.map(([key,label])=>`<section class="sales-stage" data-stage="${key}">
      ...
      <div class="sales-stage-list">${cache.opportunities.filter(o=>o.stage===key).map(o=>`<article class="sales-opportunity" draggable="true" data-id="${o.id}">
        ...
        <label class="sales-stage-picker"><span>Move stage</span><select data-move-opportunity="${o.id}" aria-label="Move ${esc(customerName(o))} to another stage">${utils.stages.map(([value,label])=>`<option value="${value}" ${value===o.stage?"selected":""}>${label}</option>`).join("")}</select></label>
    ...
    workspace.querySelectorAll("[data-move-opportunity]").forEach(select=>select.addEventListener("change",()=>moveStage(select.dataset.moveOpportunity,select.value)));
  }
  ...
  function proposals() {
    ...
    workspace.querySelectorAll("[data-proposal-pdf]").forEach(b=>b.onclick=async()=>{setMessage("Preparing proposal PDF…");try{const {data}=await window.supabaseClient.auth.getSession();const response=await fetch(`/api/proposal-pdf?id=${encodeURIComponent(b.dataset.proposalPdf)}`,{headers:{Authorization:`Bearer ${data.session?.access_token||""}`},signal:AbortSignal.timeout(15000)});if(!response.ok)throw new Error((await response.json()).error);const url=URL.createObjectURL(await response.blob()),a=document.createElement("a");a.href=url;a.download="318-proposal.pdf";a.click();URL.revokeObjectURL(url);setMessage("Proposal PDF downloaded.");}catch(e){setMessage(e.name==="TimeoutError"?"The PDF took too long. Please try again.":e.message,true);}});
  }
  function automation() {
  ...
  panel.querySelectorAll("[data-sales-view]").forEach(button=>button.onclick=()=>{view=button.dataset.salesView;panel.querySelectorAll("[data-sales-view]").forEach(b=>{const active=b===button;b.classList.toggle("active",active);b.setAttribute("aria-selected",String(active));});render();workspace.focus({preventScroll:true});});
