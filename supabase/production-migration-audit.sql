...
with required_objects(object_type, object_name, is_present) as (
  values
    ...
    ('storage bucket', 'storage.buckets.website-images', exists(select 1 from storage.buckets where id = 'website-images' and public and file_size_limit = 10485760)),
    ('RLS policy', 'storage.objects public website image reads', exists(select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'public can read website images')),
    ('RLS policy', 'storage.objects administrator uploads', exists(select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'administrators can upload website images')),
    ('RLS policy', 'public.leads administrator reads', exists(select 1 from pg_policies where schemaname = 'public' and tablename = 'leads' and policyname = 'CRM administrators can read leads')),
    ('RLS policy', 'public.website_content administrator writes', exists(select 1 from pg_policies where schemaname = 'public' and tablename = 'website_content' and policyname = 'Website administrators can manage website content')),
    ('RLS policy', 'public.menu_items administrator writes', exists(select 1 from pg_policies where schemaname = 'public' and tablename = 'menu_items' and policyname = 'Website administrators can manage menu items')),
